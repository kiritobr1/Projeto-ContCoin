...
def main(page: ft.Page):
    ...
    # Criar os controles
    label = ft.TextField(value="Clique em uma moeda para ver a cotação", read_only=True, width=300)
    
    def cotacao(e):
        moeda = e.control.text
        link = f"https://economia.awesomeapi.com.br/json/last/{moeda}"
        try:
            dados = api.get(link)
            if dados.status_code == 200:
                data = dados.json()
                # Obtém o primeiro item do dicionário (a moeda específica)
                moeda_key = list(data.keys())[0]
                valor = data[moeda_key]['bid']
                label.value = f"1 {moeda.split()[0]} = {float(valor):.4f} BRL"
                page.update()
            else:
    ...
    botoes = [ft.Button(text=moeda+"-BRL", on_click=cotacao) for moeda in moedas]
    
    # Organizar a interface
    page.add(
        ft.Column([
            label,
            *botoes  # Desempacota a lista de botões
        ])
    )
